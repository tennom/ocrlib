...
namespace ocr {
	...
	public:
		...
		unsigned int Off[128];   ///< перефирийная маска Off
		unsigned int BolS[128];  ///< вспомогательный массив  matriT,h
		char status;             ///< статус маски
		char correlation;             ///< величина корреляции маски
...
// -----------------------------------------------------------------------------
void binaryPrint(int radix, ofstream &c_out );
				 // 00000000000000001111111111111111 //

// РАСПАКОВКА на экран, представления одного целого числа int
// -----------------------------------------------------------------------------
void binaryPrint(int input, int radix, ofstream &c_out );
