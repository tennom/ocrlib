...
float GBitmap::detectSkew( unsigned int g ) {
	...
	cout<<END<<END<<"detectSkew(float angle), version V0"<<END;
	...
	//if( vmax<=((int)(3*sum/h)) ) { cout<<END<<"angle=0; no detect"<<END; return 0; }
	if( vmax<=3*sum/h ) { cout<<END<<"angle=0; no detect"<<END; return 0; }
	...
	cout<<END<<"alpha="<<alpha<<END;
	
	/*
	 cout<<"w="<<w<<"  w_8="<<w_8<<"  w2="<<w2<<"  h="<<h<<"  w*h="<<w*h<<END;
	 cout<<"  imax="<<imax<<"  vmax="<<vmax<<"  sum="<<(int)sum
	 <<"  3*sum/h="<<((int)(3*sum/h))<<END;
	 cout<<"(int)imax="<<(int)imax<<"  int(w2)="<<int(w2)<<END<<END;
...
GBitmap::toolsRadon( int sign,
	...
	////cout<<"h="<<h<<END;   if (g==24) {h=h/2; g=4;}   cout<<"h="<<h<<END; cout<<"g="<<g<<END;  //***
	...
	unsigned char *p0,*b0;     // unsigned char* input=bytes_data;
	...
	/*
	 ...
	 //scl=bytes_data + y*w;
	 for ( x=0; x < w; x++ ) {
	 // cout<<(short)bytes_data[y*w + x]<<"";
	 if(bytes_data[y*(w)+x]>128) {cout<<"1";} else{cout<<".";}
	 //if ((x)%(w/8)==0) cout<<END;
	 }
	 cout<<END;
	 }
	...
	/*
	 ...
	 for ( y=0;  y < h; y++ ){
	 cout<<(short)p1[x*h + y]<<"";
	 // if(p1[x*h+y]>1) {cout<<"1";} else{cout<<".";}
	 // if ((x)%(w/8)==0) cout<<END;     [y*(w)+x]
	 }
	 cout<<END;
	 }
	 cout<<END;
...
void GBitmap::optimizationSkew( unsigned int g ) {
	...
	cout<<END<<"optimizationSkew(unsigned int g), version V0"<<END;
	...
	cout<<"h="<<h<<END;
	if (g==24) {h=h/2; g=4;}   cout<<"h="<<h<<END; cout<<"g="<<g<<END;
	...
	unsigned char *p0,*p1,*d;     // unsigned char* input=bytes_data;
...
/*
 ...
 for ( y=0;  y < h; y++ ) {
 for ( x=0; x < w_8; x++ ) {
 //cout<<(short)bites_data[y*(w/8)+x]<<"";
 if(bites_data[y*w_8 + x]>0) {cout<<"1";} else{cout<<".";}
 //if ((x)%(w/8)==0) cout<<END;
 }
 cout<<END;
 }
...
//cout<<"sizeof(int)="<<sizeof(int)<<END;   bites_data  указатель
/*
 ...
 for ( x=0; x < w; x++ ) {
 //cout<<(short)bites_data[y*(w/8)+x]<<"";
 if(bytes_data[y*(w)+x]>0) {cout<<"1";} else{cout<<".";}
 //if ((x)%(w/8)==0) cout<<END;
 }
 cout<<END;
 }
...
/*
 ...
 for ( x=0; x < w/8; x++ ) {
 //cout<<(short)bites_data[y*(w/8)+x]<<"";
 if(bites_data[y*(w/8)+x]>0) {cout<<"1";} else{cout<<".";}
 //if ((x)%(w/8)==0) cout<<END;
 }
 cout<<END;
 }
 ...
 for ( x=0; x < w; x++ ) {
 //cout<<(short)bites_data[y*(w/8)+x]<<"";
 if(Buf[y*(w)+x]>0) {cout<<"1";} else{cout<<".";}
 //if ((x)%(w/8)==0) cout<<END;
 }
 cout<<END;
 }
...
/*
 ...
 // создание таблици для подсчета числа едениц в горизонтальном байте
 cout<<END<<END;
...
/*
 ...
 // и подсчет, с помощью таблици sumbit, числа едениц в байте
 for(y=0; y<h; y++) {   // ir
...
/*
 ...
 ///				if(sum>1024) {cout<<"1";} else{cout<<".";}
 //				 cout<<(short)((2040-sum)/255)<<"";    // (2040-sum)/256;
 } // x
 ///		cout<<END;
 } // y
 } // else
...
/*
 ...
 cout<<"vmaxF="<<vmaxF<<END;
 //double v=1<<31;  v=v*v;   cout<<"v="<<v<<END;
 */
...
/*
 ...
 ////	packImg();
 // cout<<"data_size="<<data_sze<<END;
 
 // создание массива для подсчета числа едениц в горизонтальном байте
 unsigned char* sumbit=new unsigned char[256];
 
 // заполнение таблици для подсчета числа едениц в горизонтальном байте
...
/*
 ...
 //ds1=h*(i+j);       if(ds1>nPx){cout<<"ds1="<<ds1; continue;} s1=x1+ds1; 	   // int
 //ds2=h*(i+j+step);  if(ds2>nPx){cout<<"ds2="<<ds2; continue;} s2=x1+ds2;
 
 s1=x1+h*(i+j);
 s2=x1+h*(i+j+step);
 
 // Columns-targets:  // Столбцы-целей:
 //dt1=h*(i+2*j);     if(dt1>nPx){cout<<"dt1="<<dt1; continue;} t1=x2+dt1;
 //dt2=h*(i+2*j+1);   if(dt2>nPx){cout<<"dt2="<<dt2; continue;} t2=x2+dt2;
