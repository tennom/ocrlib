//*****************************angle recognition********************************
// НЕ ВЫПОЛНЯЕТСЯ, РАБОТАЕТ GBitset.cpp
//Rotate
float GBitsetContour::detectRotation(){
//#undef DEBUGLVL   ////
maxBitAdress=nrows * ncolumns;
...
//float GBitsetContour::pageSegmentation(int* NAverage){

//return 0;

//}//_____________________________________________________________________________

void GBitsetContour::pageStart(vector<vector<int> > &INFase,
...
void GBitsetContour::contourRecognition(){
/*
        short w=ncolumns;
        short h=nrows;
...
*/
    ...
    int w=ncolumns, h=nrows;                              //      -1
    bool *p,*d,*d1,*d2,*d3,*d4,*d5;                       //       0
                                                          //  -1 0 4 0 -1
...
/**/
 ...
 void GBitsetContour::localSumStatic(vector<vector<int> > &INFase,
                              vector<vector<int> > &INGilb,
                              vector<vector<int> > &Buf1,
                              vector<vector<int> > &Buf2
                              ){
...
// INFase[16][0][nrows/2+1]=10000; INFase[16][0][nrows/2]=10000;
 ...
 void GBitsetContour::localSum(vector<vector<int> > &INFase,
                        vector<vector<int> > &INGilb,
                        vector<vector<int> > &Buf1,
                        vector<vector<int> > &Buf2
                        ){
...
void GBitsetContour::drawNFase(vector<vector<int> > &INFase, int color){
#ifdef DEBUGLVL
#ifdef DRAW
int y,x,n,in;
 TPoint ris[25000];
...
void GBitsetContour::CalculatAmplitude(vector<vector<int> > &INFase,vector<vector<int> > &INGilb){
   int y,x,n,d,p,maxA;
   long long maxAsr;
   float F,ampl;
...
//------------------------------------------------------------------------------
void GBitsetContour::deltaSlice(vector<vector<int> > &INFase,vector<vector<int> > &INGilb){
   int y,x,n;
...
//  ---------------------------------------------------------------------------
  ...
  void GBitsetContour::matExpDispers(vector<vector<int> > &INFase,
                              vector<vector<int> > &INGilb,
                              vector<int> &LFase,
                              vector<int> &LGilb
                              ){
   ...
   /*
         ...
         //     Nf = INFase[0].size();
              ...
              for ( y=1; y < Ng;  y++ ){      // y < d+1;
                DM(INGilb[0][y]<<"\t"); if (y%10==0)DM(END);  /// ТАБЛИЦА
                  }//for y
              DM(END);
...
void GBitsetContour::ScheduleProbability( vector<int> &LFase, vector<int> &LGilb,
                                   int &Porog,
                                   int &Delta ) {
...
void GBitsetContour::calculatemoments(vector<int> &LGilb,
                               float &mean,
                               float &variance,
                               float &Alpha )  {
