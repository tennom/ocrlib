...
//  ----------------------------------------------------------------------------
//  Подпрограмма выделения строк ПЕЧА ( сегментация строк - направляющие РЕЛСЫ )
//  ----------------------------------------------------------------------------
 ...
 */
		...
		for(y=2; y < nrows-2; y++){
		if(HBuf[y-1]==1) { for( x =0; x <ncolumns; x=x+4 ) drawData[0][HBuf[y]][x]*=0.1;
						   for( x =0; x <ncolumns; x=x+4 ) drawData[0][HBuf[y]-1][x]*=0.1;}
		if(HBuf[y-1]==2) { for( x =0; x <ncolumns; x=x+2 ) drawData[0][HBuf[y]][x]*=0.1;
						   for( x =0; x <ncolumns; x=x+2 ) drawData[0][HBuf[y]-1][x]*=0.1;}
	  } //for y       //0.8*ncolumns;
...
void GBitset::stringNormalisation(){
	...
	for (int index=0;index<strArray[0].size(); index++){
		//DT("//1_strArray[0]["<<index<<"].LimY0="<<strArray[0][index].LimY0-border<<" LimY1="<<strArray[0][index].LimY1-border<<" border="<<border<<END);
			int y0=nrows-strArray[0][index].LimY0-border;
			int y1=nrows-strArray[0][index].LimY1-border;
			DT("//1_LimY0="<<y0<<" LimY1="<<y1<<" nrows="<<nrows<<" border="<<border<<END);
			if(y0<0||y0>nrows)y0=0;
				if(y1<0||y1>nrows)y1=nrows;
					...
					if(y0<0||y0>nrows)y0=0; if(y1<0||y1>nrows)y1=nrows;
						...
						*/
						 DT("//2_Y0="<<y0<<" Y1="<<y1<<" nrows="<<nrows<<" border="<<border<<END);
