...
TIME_START
        ...
        // Запуск подпрограммы распознования точек в пределах строки.
                    RecognitionPoint();
...
// распознование точек внутри прямоугольников (описанных вокруг каждой точки).

   for ( Nstr=0; Nstr < strArray->size(); Nstr++ ){ // Цикл по количеству строк NStrok

         NWord = strArray[0][Nstr].wordArray.size(); // Nstr, Nwrd - глобальные
         for ( Nwrd=0; Nwrd < NWord; Nwrd++ ){ // Цикл по количеству квадратиков вдоль строки NWord
             ...
             // Вызов блока подпрограмм для распознования точек в пределах прямоугольника.
             if ( wP->stackFlag == 0 &&
         ...
         for ( Nwrd=0; Nwrd < NWord; Nwrd++ ){ // Цикл по количеству квадратиков вдоль строки NWord
             ...
             sx=(xp1-xp0)/2;   if ( sx < 8 )  sx = 8;
             //sx=sy=DeltaNstr;
             // Вызов подпрограммы маски распознования точек в пределах прямоугольника.
             if ( wP->stackFlag == 0 &&
                  yp0 > sy && yp1 < nrows-sy && xp0 > sx && xp1 < ncolumns-sx ) {
                  // Вызов подпрограммы A маски распознования точек в пределах прямоугольника.
...
*/
/*
// визуализация области распознования точки в виде 4 белох пикселей с помощью ReBuf
         for (n=0; n<ReBf; n+=m){
...
*/
/*
// визуализация области распознования точки в виде 4 белох пикселей с помощью TsertoOCR
...
///   DM(END);
   d=0;
   int NWord = strArray[0][Nstr].wordArray.size(); // Nstr, Nwrd - глобальные
   for ( Nwrd=0; Nwrd < NWord; Nwrd++ ){ // Цикл по количеству квадратиков вдоль строки NWord
         wordOCR *wT=&strArray[0][Nstr].wordArray[Nwrd];
         for (n=0; n<wT->TLline.size(); n++){
             y=wT->TLline[n].y;  x1=wT->TLline[n].x1;  x0=wT->TLline[n].x0;
...
//  ----------------------------------------------------------------------------
// Запуск подпрограммы матрици распознования точек в пределах прямоугольника.
//  ----------------------------------------------------------------------------
