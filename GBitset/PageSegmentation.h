...
//  ----------------------------------------------------------------------------
// Подпрограмма для получения координат колонок по горизонтали.
//  ----------------------------------------------------------------------------
...
void GBitset::TurnBookDetector() {
  ...
  nFrame = frameArray.size();  maXwx=MeanCol;  // *2
  if( nFrame > 1 ) {
  ...
  // TurnXL=set->TurnX;   LoadDjvu.cpp   TImageProcessorMain
...
void GBitset::MonoBookDetector() {
  ...
  int yf0,yf1,xf0,xf1;         // координаты прямоугольника вокруг вокруг всех букв скана.
  ...
  int xbMax,xbMin, ysMax,ysMin;
	 ...
	 for ( Nstr=0; Nstr < NStrok; Nstr++ ){ // Цикл по количеству строк NStrok
		 ...
		 if ( yf1 > ysMax ) { ysMax=yf1; }  // т.е. max LimY1 во всем скане
		  ...
		  Nblock = strArray[0][Nstr].blockArray.size();
...
//                     ***  **  ***    Copy    ***  **  ***
//
// Подпррограмма для помещения колонок состоящих из блоков в структуру строк.
// -----------------------------------------------------------------------------
void GBitset::CopyBlock() {

  int y,x,d;
  int xL0,xL1,yL0,yL1;    // координаты прямоугольника вокруг горизонтальных блоков
						  // описывающих многоугольники колонок
  int xc0,yc0,xc1,yc1;
  int nFrm,nFrame, nClm,nColumn;
  //int Nstr,NStrok;
  int ybMaxC,ybMinC, xbMaxS,xbMinS;  //
