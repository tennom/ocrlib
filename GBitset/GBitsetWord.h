...
void GBitset::PageWorldDet() {

  int y,x,p,m; // ,n
  int y0,y1,s0,s1;
  int res,resD,NSort,MWord;          // unsigned long; // short  // 2147483647
  MeanSA=0;
...
if( GrafT[2][0] ) { DM(END);  DM(22222<<" PageWorldDet ");  DM(END); }
...
if( GrafT[2][2] ) { WDrawVar(HBuf,ncolumns,nrows/16,scope*6,0x000000FF); }
		  ...
		  // примерная длинна строки (оценка)  //, глобальная
