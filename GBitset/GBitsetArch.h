void GBitset::readHSum(vector<int> &H){
 int y,x,d;
 short res=0;
  ...
  void GBitset::findMaxMin(vector<int> &In){
  int y,b,x;
  short dy_line;
 ...
 // }   //find max value of array
  ...
  dl[1]+=dy_line;        if(dl[1]>nrows)dl[1]=nrows;
    ...
    DM("dl[1]="<<dl[1]<<" dl[3]="<<dl[3]<<" dy_line"<<dy_line<<END);
 ...
 */
          ...
          //@//printf("max_down_res=%d  count=%d  //////*dy=%d\n" , max_down,count,dy);
             ...
             //if (count==1){
              ...
              tt=0;dlt_y=0;  //@//printf("/////////*calibration result=%d _n=%d\n", dy,n);
                          ...
                          DM(" dy="<<dy<<" baseDirection="<<baseDirection);

                                  //for ( y=0; y <h ; y=y+50 ) {
                                  //draw_y(y,9);}  // just make shcool book line

  deltaX = dlt*stepBit;  DM("deltaX="<<deltaX<<END);
}//_____________________________________________________________________________

  void GBitset::Line_Sum(vector<int> &H) { //read line sum with dlt
 int i=0;
  short tt=0;
  short res=0;
  short dlt_y=0;
 ...
 // for ( y=h-15; y < h; y++ ){lt[y]=0;}
  ...
  void GBitset::Line_Diff(vector<int> &In){//find differents
  int y,x;
  unsigned char *p2;
  In.resize(nrows);
...
*/
/*  void GBitset::draw_y(int y_str,char color){
  short tt=0;
  short dlt_y=0;
  int x,i;
              for ( x=0; x < w ; x++ ) {
...
*/
//void  GBitset::setLineCoordinate(){//new array whith line cioordinate data
//  deltaX = dlt*4;
//}//___________________________________________________________________________
