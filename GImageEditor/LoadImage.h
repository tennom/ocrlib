	...
	if(mode=="jpg"){
			...
			for (y = 0; y < h;y++){
				for (x = 0; x < wByte; x++){
					...
					*pSrc=data[0][y][x];
					//if(data[0][y][x]>120){cout<<"1";}else{cout<<"0";} //draw on screen
				}//cout<<END; //draw on screen
			}
...
#endif
	//cout<<"pechaImg1"<<pechaImg->columns()<<endl;
...
GBitmap* GImageEditor::LoadImageData(string &inputPath,bool invert){
	...
	DT("pechaImg_w="<<pechaImg_->columns()<<" pechaImg_h="<<pechaImg_->rows()<<endl);
	...
	cout <<" start load "<<inputPath<<endl;

	DT("@0");
	string str=substr(inputPath.rfind("."), 5, inputPath); cout<<" str="<<str<<endl;
	if(str==".jpg"||str==".JPG"||str==".jpeg"||str==".JPEG"){
					...
					if(w==0||h==0){cout<<"NO IMAGE"; exit(0);}
					//unsigned char lineData[wByte*h+w];
					unsigned char *srcData = [srcImageRep bitmapData];
					cout<<"@31";
					unsigned char *pSrc;
		            ...
		            if(samplesPerPixel==3)samplesPerPixel=4; //?? загадка природы
						...
						if(samplesPerPixel>2){
							...
							for (int y = 1; y < h;y++){
								...
								for (i = 0; i < wByte-samplesPerPixel; i+=samplesPerPixel){
									...
									pechaImg_[0][y][o] = grayValue;  //if(grayValue){cout<<1;}else{cout<<0;}
									o++;
								}}
						}else{
							int x,y;
...
/*void GImageEditor::LoadImageData(GBitmap* pImg,GBitmap* sImg, int x0,int y0, int w, int h){
	 ...
	 ###### some error
	if(sImg==NULL){cout<<"NO IMAGE"; return;}
	//cout<<"start"<<" x0="<<x0<<" y0="<<y0<<" w="<<w<<" h="<<h<<"sImg[0].columns()="<<sImg[0].columns()<<" sImg[0].rows()="<<sImg[0].rows()<<END;
	int print =0;
