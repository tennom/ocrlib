		...
		//if(index==strArray[0].size()-1){print=1;}else{print=0;}
		    ...
		    //
						...
						*/ 
		                ...
		                float scale;
		                if(textLineSize){
						    scale=(float)aliKali->textLineSize/textLineSize;
						}else{
							scale=1;
						}
		                GBitmap *line=GBitmap::createRegion(inputBitmap, 
															0, 
															strArray[0][index].y0-yLimit0*0.4, 
															inputBitmap->columns(), 
															strArray[0][index].y1-strArray[0][index].y0+yLimit0
		                                                    );
		                
		                 int correlation; int stepCount=0;
	ScaleDetector:;   
						if(needLineSize<3){
							scale=lineOCRScaleDetector(line,matchLine, yLimit0, yLimit1, scale,index,&correlation,3);
							bestScale=scale;
	 ...
	 DT("m0_2");
		                ...
		                //editor->WriteImageData(lineScale,"/1.jpg",0); exit(0);
	 DT("m1");
...
*/
						setMatrix->letterCorrelation(inputData,
													 ...
													 0);
						//GImageEditor *editor=GImageEditor::create();
						//editor->WriteImageData(setMatrix->drawData,"/1.jpg",0); exit(0);
	   ...
	   DT("m3_1");					//разборка строки на буквы в соответствии с величиной корреляции и вложенностью букв. например О-С
						...
						for(int m=0;m<ln;m++){
							...
							if(matchLine[m].OCRIndex==3){
							  ...
							  if(abs(yLimit0-matchLine[m].maxY)>aliKali[0][matchLine[m].letterIndex]->letterW*1.5){
							 	  matchLine[m].correlation=5;
							 	  matchLine[m].status=1;  //исключаем из рассмотрения
							  }
							  DT("/vowel/__yLimit0="<<yLimit0<<" maxY="<<matchLine[m].maxY<<" correlation="<<
...
DT("m3_5_1 space=");
		    ...
		    if(correlation+correlation/10<bestCorrelation||correlation<70){
		    ...
		    matchLine.resize(0);
			line->destroy();  //destroy text line bitmap
