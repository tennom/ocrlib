...
DP("//__________start correlation sIndex="<<sIndex<<" wIndex="<<wIndex<<" stackFlag"<<stackFlag<<END);
	...
	/////////////////////////////////////////** actual correlation////////////////////////////////////////////////////////////
	//letterInd=67;
//DC("letterBase[0].base.size()="<<letterBase[0].base.size()<<END);
...
///wordSrc=&strArray[0][sIndex].wordArray[wIndex];
  ...
  DP(" letterFlag="<<letterFlag<<" matchNum="<<matchNum<<"aliKali["<<in<<"].stackFlag="<<aliKali[in].stackFlag<<" name="<<aliKali[in].name<<" OCRStatus="<<aliKali[in].OCRStatus<<END);
  glyph=aliKali[0][in];
   ...
   if(glyph[0].Wylie!="tsheg"){ //continue;
	   if(letterFlag!=100){
			  if(letterFlag==7||letterFlag==9||letterFlag==1){
				 if(aliKali[0][in]->stackFlag==0)continue;
			  }
			  if(letterFlag==0){
